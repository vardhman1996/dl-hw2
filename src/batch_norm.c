...
matrix normalize(matrix x, matrix m, matrix v, int spatial)
{
    ...
    for(i = 0; i < x.rows; ++i){
        for(j = 0; j < x.cols; ++j){
            norm.data[i*x.cols+j] = (x.data[i*x.cols + j] - m.data[j / spatial]) / (sqrtf(v.data[j / spatial] + 0.0001));
        }
    }
    return norm;
}
...
matrix delta_mean(matrix d, matrix variance, int spatial)
{
    ...
    for (i = 0; i < d.rows; i++) 
    {
        for(j = 0; j < d.cols; j++)
        {
            dm.data[j / spatial] += d.data[i*d.cols + j] * (-1 / sqrtf(variance.data[j / spatial] + 0.0001));
        }
    }
    ...
    // for(i = 0; i < dm.cols; i++)
    // {
    //     dm.data[i] = -dm.data[i] / sqrtf(variance.data[i] + 0.0001);
    // }
    return dm;
}
...
matrix delta_variance(matrix d, matrix x, matrix mean, matrix variance, int spatial)
{
    ...
    for (i = 0; i < d.rows; i++) 
    {
        for(j = 0; j < d.cols; j++)
        {
            dv.data[j / spatial] += d.data[i*d.cols + j] * (x.data[i*d.cols + j] - mean.data[j / spatial]) * (-1 / powf(sqrtf(variance.data[j / spatial] + 0.0001), 3) / 2);
        }
    }

    // for(i = 0; i < dv.cols; i++)
    // {
    //     dv.data[i] = -dv.data[i] / powf(sqrtf(variance.data[i] + 0.0001), 3) / 2;
    // }
    return dv;
}
...
{
    matrix dx = make_matrix(d.rows, d.cols);
    int i, j, first_term, second_term, third_term;
    for(i = 0; i < d.rows; ++i){
        for(j = 0; j < d.cols; ++j){
            dx.data[i * d.cols + j] = d.data[i * d.cols + j] * (1 / (sqrtf(variance.data[j / spatial] + 0.0001))) +
                                        dv.data[j / spatial] * (2 / x.rows / spatial) * (x.data[i * d.cols + j] - mean.data[j / spatial]) +
                                        dm.data[j / spatial] * (1 / x.rows / spatial);
        }
            // first_term = d.data[i*d.cols + j] / (sqrtf(variance.data[j / spatial] + 0.0001));
            // second_term = dv.data[j / spatial] * 2 * (x.data[i*d.cols+j]-mean.data[j / spatial]) / spatial / x.rows;
            // third_term = dm.data[j/spatial] / spatial / x.rows;
